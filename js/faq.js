document.addEventListener("DOMContentLoaded", () => {
    ...
    const clearSearchButton = document.getElementById("clearFaqSearch");
    const clearAllButton = document.getElementById("clearFaqFilters");
    const resultCount = document.getElementById("faqResultCount");
    const noResults = document.getElementById("faqNoResults");
    const filters = Array.from(document.querySelectorAll(".faq-filter"));
    const items = Array.from(document.querySelectorAll(".faq-item"));
    const categorySections = Array.from(document.querySelectorAll(".faq-category-section"));
    let activeCategory = "all";

    const normalize = (value) => value.toLowerCase().replace(/[^a-z0-9\s]/g, " ").replace(/\s+/g, " ").trim();

    const setAccordion = (item, shouldOpen) => {
    ...
    const applyFilters = () => {
        ...
        items.forEach((item) => {
            ...
            const textMatch = !term || normalize(item.textContent).includes(term);
        ...
        if (resultCount) {
            if (visible === 42 && activeCategory === "all" && !term) {
                resultCount.textContent = "Showing all 42 questions";
            } else {
                resultCount.textContent = `${visible} ${visible === 1 ? "question" : "questions"} found`;
            }
        }
    };
    ...
    filters.forEach((filter) => {
        filter.addEventListener("click", () => {
            ...
            applyFilters();
            document.querySelector(".faq-content-grid")?.scrollIntoView({ behavior: "smooth", block: "start" });
        });
    });
    ...
    document.querySelectorAll("[data-open-faq]").forEach((button) => {
        button.addEventListener("click", () => {
            resetAll();
            const target = document.getElementById(button.dataset.openFaq || "");
            if (!target) return;
    ...
    const openHashQuestion = () => {
        ...
        resetAll();
        setAccordion(target, true);
        window.setTimeout(() => target.scrollIntoView({ behavior: "smooth", block: "center" }), 350);
    };
